...
return {
	...
	s(
		{
			...
			dscr = "Partial derivative with denominator only",
		},
		fmta("<>\\pdvOne{<>}", {
			f(function(_, snip)
				return snip.captures[1]
	...
	s(
		{
			...
			regTrig = true,
			snippetType = "autosnippet",
			dscr = "Partial derivative with numerator and denominator",
		},
		fmta("<>\\pdv{<>}{<>}", {
			f(function(_, snip)
				return snip.captures[1]
	...
	s(
		{
			...
			regTrig = true,
			snippetType = "autosnippet",
			dscr = "Partial derivative with numerator, denominator, and higher-order argument",
		},
		fmta("<>\\pdvN{<>}{<>}{<>}", {
			f(function(_, snip)
				return snip.captures[1]
	...
	-- Partial
	s({ trig = "ppd", snippetType = "autosnippet", dscr = "Partial symbol" }, {
		t("\\partial "),
	}, { condition = tex.in_mathzone }),
	...
	-- Limit
	s(
		{ trig = "([^%a])limt", wordTrig = false, regTrig = true, snippetType = "autosnippet" },
		fmta("<>\\lim_{<>\\to <>}", {
			f(function(_, snip)
				return snip.captures[1]
			end),
	...
	--
	-- Prime, i.e. '
	s({ trig = ";'", snippetType = "autosnippet", dscr = "Prime symbol" }, {
		t("\\prime"),
	}, { condition = tex.in_mathzone }),
	...
	s(
		{ trig = "([^%a])oit", wordTrig = false, regTrig = true, snippetType = "autosnippet" },
		fmta("<>\\oint_{<>}^{<>}", {
			f(function(_, snip)
				return snip.captures[1]
			end),
	...
	s({ trig = ",.", snippetType = "autosnippet", dscr = "Dot product" }, {
		t("\\cdot "),
	}),
	...
	-- <= (minor than or equal to)
	s({ trig = "<=", snippetType = "autosnippet", dscr = "Greater than or equal to" }, {
		t("\\leq "),
	}),
